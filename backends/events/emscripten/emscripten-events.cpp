...
bool EmscriptenEventSource::pollEvent(Common::Event &event) {
...
// filter mouse events (touch)
// filter mouse events w/o mouse motion (mouse like behavior)

void EmscriptenEventSource::preprocessEvents(SDL_Event *event) {

	if (emFilterMouseEvents) {
		switch (event->type) {
...
void EmscriptenEventSource::preprocessFingerDown(SDL_Event *event) {
...
void EmscriptenEventSource::preprocessFingerUp(SDL_Event *event) {
	// front (0) or back (1) panel
	SDL_TouchID port = 0;
	// id (for multitouch)
	SDL_FingerID id = event->tfinger.fingerId;
		...
		if (_finger[port][i].id == id) {
			...
			if (!_multiFingerDragging[port]) {
				if ((event->tfinger.timestamp - _finger[port][i].timeLastDown) <= MAX_TAP_TIME) {
					...
					float xrel = ((event->tfinger.x * (float) getTouchScreenWidth()) - (_finger[port][i].lastDownX * (float) getTouchScreenWidth()));
					float yrel = ((event->tfinger.y * (float) getTouchScreenHeight()) - (_finger[port][i].lastDownY * (float) getTouchScreenHeight()));
...
void EmscriptenEventSource::preprocessFingerMotion(SDL_Event *event) {
	// front (0) or back (1) panel
	SDL_TouchID port = 0;
	// id (for multitouch)
	SDL_FingerID id = event->tfinger.fingerId;
...
void EmscriptenEventSource::convertTouchXYToGameXY(float touchX, float touchY, int *gameX, int *gameY) {
	int screenH = _graphicsManager->getWindowHeight();
	int screenW = _graphicsManager->getWindowWidth();

	const int dispW = getTouchScreenWidth();
	const int dispH = getTouchScreenHeight();
...
void EmscriptenEventSource::finishSimulatedMouseClicks() {
	for (int port = 0; port < SCE_TOUCH_PORT_MAX_NUM; port++) {
		for (int i = 0; i < 2; i++) {
